...
function content5(data){
	...
	if(data.hasOwnProperty("boards")){
		$.each(data.boards, function(i,j) {
			...
			temp+="<td>"+j.outtmp+" ℃</td>";
			// if(j.state==true)
				// temp+="<td class='colorGreencss'>"+t("展开")+"&nbsp;<div class='broadstatuscss statusOncss' data-id='no"+j.no+"' ></div></td>";
			// else
				// temp+="<td class='colorRedcss'>"+t("展开")+"&nbsp;<div class='broadstatuscss statusOffcss' data-id='no"+j.no+"' ></div></td>";
			
			var chiphtml="";
			// for(var k=0;k<j.chipnum;k++){
				// var st="normal";
				 
				// if(j.false!=""){
					// $.each(j.false, function(m,n) {
						// if(n==k+1){
							// st="abnormal";
							// return false;
						// }
					// });
				// }
				// chiphtml+="<div class='boardschip "+st+"'></div>";
			// }		
			// chiphtml="<div class='boardschipdiv'>"+chiphtml+"</div>";
			
			temp="<tr>"+temp+"</tr>"+"<tr class='no"+j.no+" hidden'><td colspan=2>"+t("算力板")+""+j.no+"-"+t("芯片状态")+"</td><td colspan=7 style='width:100px;'>"+chiphtml+"</td></tr>";
